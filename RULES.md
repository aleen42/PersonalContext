...
- **Object Alignment**: **Align** colons in object properties across multiple lines.
    * Example:
      ...
      ```
- **No Space Inside Brackets/Parens**: Do NOT add spaces inside array brackets or function parentheses (e.g., `[1, 2]`, `(a, b) => {}`).
...
- **ES6 Syntax**: **Maximize the use of ES6+ syntax** (e.g., arrow functions, destructuring, default parameters, spread/rest operators).
- **Variables**: **Always** use `const` by default, and `let` only when reassignment is needed. **DO NOT** use `var`.
- **Functions**: Prefer **arrow functions** for anonymous functions and callbacks. Omit parentheses for single-parameter arrow functions (e.g., `x => {}`). Use parentheses for zero or multiple parameters (e.g., `() => {}`, `(x, y) => {}`).
